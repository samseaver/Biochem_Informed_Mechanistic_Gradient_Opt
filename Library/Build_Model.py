...
def get_V0(inputs, parameter, targets, lower_bounds, trainable, V0_init=-2, verbose=False):
    ...
    -2  "evidence-only". THE PUBLISHED SETTING, hard-coded in predict_bioinformed_flux.py.
        ...
        * each media chain (exchange -> e0/c0 transporter -> c0/d0
          ...
          it automatically at the start of each run.
    ...
    # the opposing column held at 0. Built by Library/Media_Chain.build, below.
    if V0_init == -2:
        # The table is built here rather than expected to exist. It is derived
        # entirely from integration_results/fva.tsv, costs a fraction of a
        # second, and rebuilding it every run means it cannot go stale against
        # a re-run FVA -- which would otherwise seed the media layer from an
        # old capacity envelope without saying so.
        #
        # output_dir is <project>/ml[/test]/svp_X.X/ -- walk up to the project
        # folder, the one holding integration_results/ alongside ml/.
        project_dir, probe = None, os.path.abspath(parameter.output_dir)
        while probe != os.path.dirname(probe):
            if os.path.isdir(os.path.join(probe, "integration_results")):
                project_dir = probe
                break
            probe = os.path.dirname(probe)
        if project_dir is None:
            raise FileNotFoundError(
                "V0_init=-2 needs a project directory with integration_results/ "
                f"at or above {parameter.output_dir}. Run generate_feasible_flux.py "
                "to produce the capacity envelope first.")
        if not os.path.exists(os.path.join(project_dir, "integration_results", "fva.tsv")):
            raise FileNotFoundError(
                f"V0_init=-2 needs {project_dir}/integration_results/fva.tsv. "
                "Run generate_feasible_flux.py first.")

        from Library.Media_Chain import build as _build_media_chain
        _build_media_chain(project_dir)
        chain_file = os.path.join(project_dir, "integration_results",
                                  "media_chain_init.tsv")
