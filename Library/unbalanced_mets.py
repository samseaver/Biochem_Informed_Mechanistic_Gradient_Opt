...
sname = sName[spc]
model_name = f"{sName[species[0]]}_plastid_Thylakoid_Reconstruction_ComplexFix_070224_duplicated.xml"

model_name = f"{sName[species[0]]}_plastid_Thylakoid_Reconstruction_ComplexFix_RevFix3_250512_duplicated.xml"
...
pred_path = os.path.join(saveTo, f"{spc}_Leaf_{day}_complexFix{more}_V_rxn_trmt_fba_Vbf_RS.tsv")

chlorophyll_enzymes_file = os.path.join(project_root, "Result", resultFoldr, "cloro_rxns.tsv")
...
def flux_scatter_cyto(netFlux_df, treatments):
	...
	for spc in species:
			## scatter plot 
		fig = make_subplots(rows=len(treatments), cols=len(days),
					# specs=specs, 
					...
					subplot_titles=[f'{trmt} - {ts}' for trmt in treatments for ts in days])
		col = 0
		for ts in days:
			col += 1
			row = 0 
...
def net_flux_cdfs(netFlux_df, clm, treatments=['Control', 'FeLim']):
	...
	fig, axes = plt.subplots(len(days), 1, figsize=(14, 6), sharey=True, sharex=True)
...
def net_flux_CDF_plotly(netFlux_df, clm, treatments=['Control', 'FeLim']): 
	...
	fig = make_subplots(
		rows=1, cols=len(days),  # 1 row, 2 columns for 'd1' and 'd2'
		shared_xaxes=True,
		shared_yaxes=True,
		subplot_titles=[f'CDF for {ts}' for ts in days]
	)
...
def read_metabolite_netFluxes():
	...
	for spc in species:
		for trmt in treatments:
			for ts in days:
				matrix_path = os.path.join(saveTo, "treatment_matrices", f"{spc}_{trmt}_{ts}_s_matrix.tsv")
				df = pa.read_csv(matrix_path, sep='\t')
				
				df.rename(columns={'Unnamed: 0': 'met_ID', 'sum': 'netFlux', "sum_c":'C_netFlux'} , inplace=True)  # Rename the new 'index' column to 'id'
...
def read_reaction_predFlux(pref_path): 
	predFlux_df = pa.DataFrame()
	for spc in species:
		# for trmt in treatments:
		if day == 'all': 
			df = pa.read_csv(pref_path, sep='\t')
			df = df[["rxn_ID", "treatment", "Pred"]]
			df[['treatment', 'time_stamp']] = df['treatment'].str.split('_', expand=True)
			predFlux_df = df
			predFlux_df['spc'] = spc
			predFlux_df.rename(columns={'Pred': 'predFlux'}
						, inplace=True) 
...
if __name__ == '__main__':
	...
	predFlux_df = read_reaction_predFlux(pred_path)
	plot_chlorophill_lines(predFlux_df)

	chlorophyll_rxns = ["rxn19846_d0", "rxn01629_d0", "rxn00029_d0", "rxn00060_d0", 
	"rxn02264_d0", "rxn02288_d0", "rxn02303_d0", "rxn02304_d0", "rxn02733_d0", 
	"rxn02959_d0", "rxn04152_d0", "rxn04153_d0", "rxn04154_d0"]
