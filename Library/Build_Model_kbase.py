...
def Loss_SV(V, S, gradient=False, save=False):
    ...
    # Loss_norm = ops.sum(Loss, axis=1, keepdims=True)# /S.shape[0] # rescaled
...
def Loss_all5(V, Vin, Vout, Vlb, parameter, gradient=False):
...
def Gradient_Descent(V, Vin, Vout, Vlb, parameter, mask, trainable=True, history=False, verbose=False):
    ...
    for t in range(1, parameter.timestep+1):  # Update V with GD
        ...
        L, dL = Loss_all(V, Vin, Vout, Vlb, parameter, gradient=True, p_sv = 2, save=save)
        save = False
...
class Neural_Model:
    ...
    def get_parameter(self, biomass_max=4.0, verbose=False):
        ...
        # bio_id = rxns.index('bio1_biomass') #373 # sandbox 35 -- full model 342 373 thylakoid
