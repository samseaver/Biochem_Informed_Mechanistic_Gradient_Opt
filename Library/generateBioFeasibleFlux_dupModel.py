...
spc, day, grpr3, FVA = 'athaliana', 'all', 'TSU', 'atha_full_model' #'C24' or 'TSU'
spc, day, grpr3, FVA = 'athaliana', 'ZT9', 'TSU', 'atha_full_model'


## FVA fluxes
fluxes_file = f"/Users/selalaoui/Projects/AMN/omic_amn_mm/Dataset_input/FVA_Output_{FVA}.tsv"


if 'atha' in spc:
    ## new thylakoid fluxes
    ctrl_trmt = 'CTL'
    scores_file = f"/Users/selalaoui/Projects/QPSI_project/Enzyme_Abundance_all/integration_results/secMetResults/Atha_objective_abundance_{ctrl_trmt}.tsv"
    treatments = ['FRZ']
    value_col = 'value'
    other_colm = 'genotype'
else:
    ...
    ## Objective abundance
    scores_file = f"/Users/selalaoui/Projects/QPSI_project/Enzyme_Abundance_all/integration_results/reaction_scores_binding_Oct9/plastidial_model/{spc}_objective_abundance_{ctrl_trmt}.tsv"
...
def load_fluxes(fluxes_file, all_rxn=True, verbose=True):
    if all_rxn:
...
def load_scores(ctrl_trmt= 'Control', value_col='value', verbose=False):
    ...
    if (grpr3 in ['C24', 'TSU']) and (day == 'all'):
...
def generate_all_df(treatments, value_col='value', trmt_column='treatment', isDup=False, verbose=False):
    ...
    if isDup:
        ...
        fluxes_dup[flx_rxn_ID] = fluxes_dup.apply(lambda row: get_rxn_ID(row), axis=1)
        print("fluxes_dup: \n", fluxes_dup.head())
        flx_rxn_ID = 'rxn_ID_only'
    

    # replace reaction ID in scores DF by the duplicated scores and duplicate entries for
    ...
    scores_df = scores_df.merge(fluxes_dup[['rxn_ID_only', 'rxn_ID']], left_on='rxn_ID',
                                right_on=flx_rxn_ID, how=how, suffixes=('_l', ''))
    ...
    all_control = control.merge(fluxes_dup, left_on='rxn_ID', right_on=flx_rxn_ID,
                                    how=how, suffixes=('_l', ''))
    ...
    # else:
    name = [spc, 'complexFix', grpr3, day, 'noADP', 'Vbf', 'maxCtrl']

    if 'relab' in scores_file:
        name = name + ['relab', 'mixedRelab']
    if 'atha' in spc:
        name = name + ['fullmodel']

    all_control[ls].to_csv("/Users/selalaoui/Projects/AMN/omic_amn_mm/Dataset_input/"+"_".join(name)+".csv", index=False)

    all_control.to_csv("/Users/selalaoui/Projects/AMN/omic_amn_mm/Dataset_input/"+"_".join(name+['kapp'])+".csv", index=False)
...
def all_kapp(value_col, isDup=False):
    ...
    scores_df = scores_df.merge(fluxes_dup, left_on='rxn_ID',
                                right_on=flx_rxn_ID, how='left', suffixes=('_l', ''))
