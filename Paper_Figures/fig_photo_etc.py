...
def _analysis_cache() -> str:
    ...
    here. Rebuild them with analysis_scripts/build_analysis_cache.sh, or point
    the env var at an existing copy.
    """
    import io_utils
...
class CONFIG:
    ...
    # analysis_scripts/glk_regulon_summary.py) we overlay the top
    ...
    # find_phenotype_correlate.py (an off-figure ranking) — top 3 by Pearson r with
    # the ETC log2 trajectory (water-status indices), plus Chl and Fe54
    # as biology-required references.
    # Measured phenotype data, not generated by this repository. Tracked at
    # data/phenotype/ -- see its README. BIOFLUX_PHENOTYPE_FILE overrides.
...
def _load_ratelimiting_genes(species_name: str
    ...
    analysis_scripts/glk_regulon_summary.py plus the raw TMM file."""
